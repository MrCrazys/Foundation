...
public final class LagCatcher {
	...
	public static void performanceTest(int cycles, String name, Runnable code) {
		ValidCore.checkBoolean(cycles > 0, "Cycles must be above 0");
		...
		CommonCore.log("Test '" + name + "' took " + MathUtilCore.formatTwoDigits(finishAndCalculate(name + "-whole")) + " ms. Average " + MathUtilCore.average(lagMap) + " ms");
		...
		if (!durationsMap.isEmpty()) {
			for (final Map.Entry<String, List<Long>> entry : durationsMap.entrySet()) {
				...
				CommonCore.log("\tSection '" + section + "' took " + MathUtilCore.formatTwoDigits(duration / 1_000_000D));
			}

			CommonCore.log("Section measurement ended.");
	...
	public static void performancePartSnap(String section) {
		ValidCore.checkBoolean(durationsMap.containsKey(section), "Section " + section + " is not measured! Are you calling it from performanceTest?");
