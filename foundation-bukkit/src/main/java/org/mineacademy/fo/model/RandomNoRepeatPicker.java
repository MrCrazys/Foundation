...
public abstract class RandomNoRepeatPicker<T> {
	...
	public void setItems(final Iterable<T> list) {
		ValidCore.checkBoolean(list != null && list.iterator().hasNext(), "Cannot set items to an empty list!");
	...
	public T pickRandom(final Player player) {
		...
		while (!this.list.isEmpty()) {
			final T picked = this.list.remove(RandomUtilCore.nextInt(this.list.size()));
