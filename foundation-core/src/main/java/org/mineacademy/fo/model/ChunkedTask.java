...
public abstract class ChunkedTask {
		...
		Platform.runTask(0, () -> {
			...
			for (int i = this.currentIndex; i < this.currentIndex + this.processAmount; i++) {
				...
				try {
					...
					CommonCore.error(t, "Error in " + this + " processing index " + processed);
			...
			if (processed > 0 || !finished)
				CommonCore.log(this.getProcessMessage(now, processed));

			if (!finished) {
				this.currentIndex += this.processAmount;

				Platform.runTaskAsync(this.waitPeriodTicks, this::startChain);
	...
	public final void cancel() {
		ValidCore.checkBoolean(this.processing, "Chunked task is not running: " + this);
