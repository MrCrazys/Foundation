...
class NMSDragon_v1_6 extends NMSDragon {
	...
	public Object getSpawnPacket() {
		...
		try {
			...
			final Field a = ReflectionUtil.getField(mob_class, "a");
			a.setAccessible(true);
			a.set(mobPacket, EntityID);// Entity ID

			final Field b = ReflectionUtil.getField(mob_class, "b");
			b.setAccessible(true);
			b.set(mobPacket, EntityType.ENDER_DRAGON.getTypeId());// Mob type

			// (ID: 64)
			final Field c = ReflectionUtil.getField(mob_class, "c");
			c.setAccessible(true);
			c.set(mobPacket, this.getX());// X position

			final Field d = ReflectionUtil.getField(mob_class, "d");
			d.setAccessible(true);
			d.set(mobPacket, this.getY());// Y position

			final Field e = ReflectionUtil.getField(mob_class, "e");
			e.setAccessible(true);
			e.set(mobPacket, this.getZ());// Z position

			final Field f = ReflectionUtil.getField(mob_class, "f");
			f.setAccessible(true);
			f.set(mobPacket, (byte) (int) (this.getPitch() * 256.0F / 360.0F));// Pitch

			final Field g = ReflectionUtil.getField(mob_class, "g");
			g.setAccessible(true);
			g.set(mobPacket, (byte) 0);// Head

			// Pitch
			final Field h = ReflectionUtil.getField(mob_class, "h");
			h.setAccessible(true);
			h.set(mobPacket, (byte) (int) (this.getYaw() * 256.0F / 360.0F));// Yaw

			final Field i = ReflectionUtil.getField(mob_class, "i");
			i.setAccessible(true);
			i.set(mobPacket, this.getXvel());// X velocity

			final Field j = ReflectionUtil.getField(mob_class, "j");
			j.setAccessible(true);
			j.set(mobPacket, this.getYvel());// Y velocity

			final Field k = ReflectionUtil.getField(mob_class, "k");
			k.setAccessible(true);
			k.set(mobPacket, this.getZvel());// Z velocity

			final Object watcher = this.getWatcher();
			final Field t = ReflectionUtil.getField(mob_class, "t");
	...
	public Object getDestroyPacket() {
		...
		try {
			...
			final Field a = ReflectionUtil.getField(packet_class, "a");
			a.setAccessible(true);
	...
	public Object getMetaPacket(Object watcher) {
		...
		try {
			...
			final Field b = ReflectionUtil.getField(packet_class, "b");
			b.setAccessible(true);
	...
	public Object getTeleportPacket(Location loc) {
		...
		try {
			...
			final Field c = ReflectionUtil.getField(packet_class, "c");
			c.setAccessible(true);
			c.set(packet, (int) Math.floor(loc.getY() * 32.0D));

			final Field d = ReflectionUtil.getField(packet_class, "d");
			d.setAccessible(true);
			d.set(packet, (int) Math.floor(loc.getZ() * 32.0D));

			final Field e = ReflectionUtil.getField(packet_class, "e");
			e.setAccessible(true);
			e.set(packet, (byte) (int) (loc.getYaw() * 256.0F / 360.0F));

			final Field f = ReflectionUtil.getField(packet_class, "f");
			f.setAccessible(true);
