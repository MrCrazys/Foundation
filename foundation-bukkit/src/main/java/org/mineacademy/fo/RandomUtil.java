...
public final class RandomUtil extends RandomUtilCore {
	...
	public static DyeColor nextDyeColor() {
		return DyeColor.values()[getRandom().nextInt(DyeColor.values().length)];
	}
	...
	public static Location nextLocation(final Location origin, final double radius, final boolean is3D) {
		final double rectX = getRandom().nextDouble() * radius;
		final double rectZ = getRandom().nextDouble() * radius;
		final double offsetX;
		final double offsetZ;
		double offsetY = 0;
		final int transform = getRandom().nextInt(4);
		if (is3D) {
			final double rectY = getRandom().nextDouble() * radius;
	...
	public static Location nextLocation(final Location origin, final double minRadius, final double maxRadius, final boolean is3D) {
		...
		final double rectX = getRandom().nextDouble() * (maxRadius - minRadius) + minRadius;
		final double rectZ = getRandom().nextDouble() * (maxRadius + minRadius) - minRadius;
		final double offsetX;
		final double offsetZ;
		...
		final int transform = getRandom().nextInt(4);
		if (is3D) {
			final double rectY = getRandom().nextDouble() * (maxRadius + minRadius) - minRadius;
			offsetY = getYCords(transform, rectY);
		}
		if (transform == 0) {
			offsetX = rectX;
	...
	private static double getYCords(int transform, double rectY) {
		...
		final double nextY = getRandom().nextDouble();
