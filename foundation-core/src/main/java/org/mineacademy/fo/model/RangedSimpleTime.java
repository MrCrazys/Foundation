...
public final class RangedSimpleTime implements ConfigStringSerializable {
	...
	public RangedSimpleTime(final SimpleTime min, final SimpleTime max) {
		ValidCore.checkBoolean(min.getTimeTicks() >= 0 && max.getTimeTicks() >= 0, "Values may not be negative");
		ValidCore.checkBoolean(min.getTimeTicks() <= max.getTimeTicks(), "Minimum must be lower or equal maximum");
	...
	public int getRandomTicks() {
		return RandomUtilCore.nextBetween(this.min.getTimeTicks(), this.max.getTimeTicks());
	}
	...
	public int getRandomSeconds() {
		return RandomUtilCore.nextBetween((int) this.min.getTimeSeconds(), (int) this.max.getTimeSeconds());
	}
